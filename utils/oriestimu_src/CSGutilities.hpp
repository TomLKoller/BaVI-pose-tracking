...
//! Contact: {seel,ruppin}@control.tu-berlin.de
	...
	void normalize(const  double vec[], int length, double vecNorm, double normalizedResult[]);


    //! Calculate the vector product of a and b
	void vectorCrossProduct(const double a[R3],const  double b[R3], double result[R3]);


    //! Standard scalar product: a[0]*b[0] + a[1]*b[1] + a[2]*b[2]
	double scalarProduct(const double a[R3],const  double b[R3]);
#endif
